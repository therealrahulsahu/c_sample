...
void main(){
	int gd=DETECT, gm, type, trans, tx, ty, angle, sx, sy, valid=1, x1, y1, x2, y2, rad;
	float t_mat[3][3]={0}, data[3][3]={0}, result[3][3]={0};
	...
	if(type==1){
		...
		if(trans!=1)
			...
			circle(x1+rad*sx, y1+rad*sy, rad);
	}else{
		printf("Invalid Input\n");
	}
	getch();
