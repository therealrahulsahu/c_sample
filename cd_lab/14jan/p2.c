...
int main()
{
    ...
    FILE *key = fopen("D:\\Rahul Code\\C Git\\c_sample\\cd_lab\\14jan\\key.txt", "w");
    FILE *idf = fopen("D:\\Rahul Code\\C Git\\c_sample\\cd_lab\\14jan\\idf.txt", "w");
    char str[35];
    while(fscanf(fp, "%s", str)!=EOF){
        if(isvalidKeyword(str))
            fprintf(key, "Keyword : %s\n", str);
